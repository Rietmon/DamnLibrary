...
namespace Rietmon.Management
{
    public static class AddressableManager
    {
        ...
        public static async Task<Prefab<WindowBehaviour>> GetWindowPrefabAsync(string windowName) =>
            Internal_VerifyAsset(
                await Internal_GetGameObjectComponent<WindowBehaviour>(PathToDataWindows.Format(windowName)),
                windowName);

        public static async Task<SpriteAtlas> GetSpriteAtlas(string atlasPath) =>
            Internal_VerifyAsset(await Internal_GetAssetAsync<SpriteAtlas>(PathToSpritesAtlases.Format(atlasPath)),
                atlasPath);
        ...
        public static async Task<AudioClip> GetAudio(string audioName)
        {
            ...
            Debug.LogError(
                $"[{nameof(AddressableManager)}] ({nameof(GetAudio)}) Unable to find audio with name {audioName}. Check extension!");
            return null;
        }
        ...
        public static async Task<T> Internal_GetAssetAsync<T>(string assetName) where T : Object
        {
            ...
            {
                Debug.LogError(
                    $"[{nameof(AddressableManager)}] ({nameof(Internal_GetAssetAsync)}) Unable to find the location with the name {assetName}");
                return null;
            }

            var result = await Addressables.LoadAssetAsync<T>(locations[0]).Task;
            Debug.Log(
                $"[{nameof(AddressableManager)}] ({nameof(Internal_GetAssetAsync)}) Asset with the name {assetName} was loaded in {Time.frameCount - startLoadingFrame} frames.");
            return result;
        }
        ...
        public static T Internal_VerifyAsset<T>(T asset, string assetName) where T : Object
        {
            ...
            Debug.LogError(
                $"[{nameof(AddressableManager)}] ({nameof(Internal_VerifyAsset)}) Error at loading asset with the name {assetName}. Result equal a null.");
            return default;
        }
    }
}
